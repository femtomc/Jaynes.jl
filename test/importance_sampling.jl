...
@testset "Importance sampling" begin
    z = 3.0
    observations = Jaynes2.selection((:z, z))
    n_traces = 5

    @testset "Linear Gaussian model" begin
        calls, lnw, lmle = Jaynes2.importance_sampling(LinearGaussian, (0.0, 1.0); observations = observations, num_samples = n_traces)
        @test length(calls) == n_traces
        @test length(lnw) == n_traces
        @test isapprox(Jaynes2.lse(lnw), 0., atol = 1e-9)
        @test !isnan(lmle)
        for call in calls
            @test call[:z] == z
        end
    ...
    @testset "Linear Gaussian proposal" begin
        calls, lnw, lmle = Jaynes2.importance_sampling(LinearGaussian, (0.0, 1.0), LinearGaussianProposal, (); observations = observations, num_samples = n_traces)
        @test length(calls) == n_traces
        @test length(lnw) == n_traces
        @test isapprox(Jaynes2.lse(lnw), 0., atol = 1e-9)
        @test !isnan(lmle)
